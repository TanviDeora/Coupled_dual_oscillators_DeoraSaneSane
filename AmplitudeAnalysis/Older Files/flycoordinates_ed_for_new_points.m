...
for i=1:3:S(1,2)
for j=1:S(1,1)
t(j,i)= data(j,i)-data(j,ind_f*3-2);
end
end

for i=2:3:S(1,2)
for j=1:S(1,1)
t(j,i)= data(j,i)-data(j,ind_f*3-1);
end
end

for i=3:3:S(1,2)
for j=1:S(1,1)
t(j,i)= data(j,i)-data(j,ind_f*3);
end
end
...
for j=1:S(1,1)
xdir(j,:)=[t(j,ind_rwb*3-2)-t(j,ind_lwb*3-2), t(j,ind_rwb*3-1)-t(j,ind_lwb*3-1), t(j,ind_rwb*3)-t(j,ind_lwb*3)];
...
%points left and right wing base and left wing base and head junction
%line23(j,:)=[t(j,7)-t(j,4), t(j,8)-t(j,5), t(j,9)-t(j,6)];
lineplane(j,:)=[t(j,ind_lwb*3-2)-t(j,ind_f*3-2), t(j,ind_lwb*3-1)-t(j,ind_f*3-1), t(j,ind_lwb*3)-t(j,ind_f*3)];
n(j,:)=cross(xdir(j,:),lineplane(j,:));
